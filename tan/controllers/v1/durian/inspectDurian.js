...
export const inspectDurian = async (req, res) => {
  try {
    ...
    let durian = null;
    if (mongoose.Types.ObjectId.isValid(displayId)) durian = await Durian.findById(displayId).lean();
    if (!durian) durian = await Durian.findOne({ displayId }).lean();
    ...
    if (!durian) {
      if (mongoose.Types.ObjectId.isValid(displayId)) {
        lot = await Lot.findById(displayId);
      }
      if (!lot) {
        lot = await Lot.findOne({ displayId });
      }
      if (!lot) {
      ...
      lot = await Lot.findById(durian.lotId);
      if (!lot) {
    ...
    const shipping = await Shipping.findById(lot.shippingId);
    if (!shipping) {
    ...
    if (durian) await Durian.deleteOne({ _id: durian._id });
